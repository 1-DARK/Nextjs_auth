...
export default function Loginpage() {
    ...
    const [shownum, setnum] = React.useState(false);
    const [loading, setLoading] = React.useState(false);
    const onPIN = async () => {
        try {
            setLoading(true);
            const response = await axios.post("/api/users/forgot-pass", user)
    ...
    useEffect(() => {
        if (user.email.length > 0 && user.security_pin.length > 0) {
            setbuttondis(false);
        } else {
            setbuttondis(true);
        }
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 px-4">
            <div className="w-full max-w-md bg-gray-800 rounded-xl shadow-xl p-8 space-y-6 border border-gray-700">
                <h1 className="text-3xl font-bold text-center text-white">{loading ? "Processing" : "Enter Email and Pin"}</h1>
                ...
                <div className="space-y-2 relative">
                    <label htmlFor="password" className="block text-sm font-medium text-gray-300">Enter 6 digit Security PIN</label>
                    <div className="relative">
                        <input
                            ...
                            onChange={(e) => setuser({ ...user, security_pin: e.target.value })}
